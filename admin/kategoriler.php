					...
					if(!empty($_POST['kategori_baslik']) || !empty($_POST['kategori_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET kategori_baslik = ?, kategori_seo = ?, kategori_aciklama = ?,  kategori_kisaaciklama = ?, kategori_resim = ?, dil_id = ?, kategori_tipi = ?, kategori_durum = ?, altkategori_id = ?, kategori_ust_id = ?");
								$insert->execute(array($kategori_baslik[$key], Seo_Link_Cevir($kategori_baslik[$key]), $kategori_aciklama[$key], $kategori_kisaaciklama[$key], $kategori_resim, $key, 1, $kategori_durum, $altkategori_id, $LastID));
													...
													<div class="row">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="kategori_kisaaciklama[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Kısa Açıklama" required="">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
					...
					if(isset($submitControl)){
						if(!empty($_POST['kategori_baslik']) || !empty($_POST['kategori_aciklama'])) {
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  kategori_baslik = ?, kategori_seo = ?, kategori_aciklama = ?, kategori_kisaaciklama = ?, kategori_resim = ?, kategori_tipi = ?, altkategori_id = ?, kategori_durum = ? WHERE kategori_ust_id = ? AND dil_id = ?");
									$update->execute(array($kategori_baslik[$key], Seo_Link_Cevir($kategori_baslik[$key]), $kategori_aciklama[$key], $kategori_kisaaciklama[$key], $kategori_resim, 1, $altkategori_id, $kategori_durum, $id, $key));
								}
							}
							if ($update) {
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Kısa Açıklama</label>
																<input type="text" name="kategori_kisaaciklama[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Kısa Açıklama" required="" value="<?php echo GetTableValue("kategori_kisaaciklama",TABLE,"where kategori_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
